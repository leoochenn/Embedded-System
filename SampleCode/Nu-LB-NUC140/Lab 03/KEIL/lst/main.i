...
guess[i]=keyin;
sprintf(Text+7,"%d%d%d%d", guess[0],guess[1],guess[2],guess[3]); 
print_Line(1, Text);         
...
void winSound(){
Buzz(1);
CLK_SysTickDelay(3*1000000);
Buzz(3);
CLK_SysTickDelay(1*1000000);
Buzz(1);

for(x = 0; x < 3; x++){ CLK_SysTickDelay(1000000-100000); }
Buzz(1);
CLK_SysTickDelay(1*1000000);
Buzz(1);
}
...
void getUserInput(uint8_t keyin, uint8_t* code, char *Text5, int* Code){
	...
	while(1){
		...
		code[i] = keyin;
		sprintf(Text5,"%d%d%d%d", code[0],code[1],code[2],code[3]); 
		print_Line(1, Text5);         
		CLK_SysTickDelay(500000);   
		keyin = 0;
		++i;
